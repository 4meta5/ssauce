...
decl_storage! {
    trait Store for Module<T: Trait> as TransferTax {
...
mod tests {
    ...
    impl std::convert::From<()> for TestEvent {
        fn from(unit: ()) -> Self {
            TestEvent::transfer_tax(RawEvent::NullEvent(6))
        }
    }
    ...
    fn run_to_block(n: u64) {
        while System::block_number() < n {
            TransferTax::on_finalize(System::block_number());
    ...
    fn new_test_ext_behaves() {
        new_test_ext().execute_with(|| {
            ...
            assert!(TransferTax::is_on_council(&first_account));
            assert!(TransferTax::is_on_council(&second_account));
            assert!(TransferTax::is_on_council(&third_account));
            assert!(TransferTax::is_on_council(&fourth_account));
            assert!(TransferTax::is_on_council(&fifth_account));
            assert!(TransferTax::is_on_council(&sixth_account));
            assert!(TransferTax::is_on_council(&seventh_account));
        })
    }
    ...
    fn transfer_reserves_tax() {
        new_test_ext().execute_with(|| {
            ...
            let second_account = ensure_signed(Origin::signed(2)).unwrap();
            assert_err!(
                TransferTax::request_transfer(Origin::signed(3), first_account, 1),
                "Must be able to pay tax to make transfer"
            );
            TransferTax::request_transfer(Origin::signed(1), second_account, 8);
            ...
            assert!(TransferTax::transfer_requests()
                .iter()
                .any(|a| *a == mock_spend_request));

            // check that user debt is correctly tracked
            assert_eq!(TransferTax::user_debt(&first_account).unwrap(), 8,);

            // check that the correct event is emitted
            let expected_event = TestEvent::transfer_tax(RawEvent::TransferRequested(
                first_account.clone(),
    ...
    fn propose_treasury_spend_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_err!(
                TransferTax::propose_treasury_spend(Origin::signed(8), first_account, 10u64.into()),
                "must be on council to make proposal"
            );
            System::set_block_number(5);
            TransferTax::propose_treasury_spend(Origin::signed(1), eighth_account, 10u64.into());
            ...
            assert_eq!(
                TransferTax::proposals(first_account).unwrap(),
                expected_proposal
            );

            let expected_event = TestEvent::transfer_tax(RawEvent::TreasuryProposal(
